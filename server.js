...
function payRoulette(choice, amt, r) {
  if (choice==='do'      && r.color==='red')             return amt*2;
  if (choice==='den'     && r.color==='black')           return amt*2;
  if (choice==='chan'    && r.isEven)                    return amt*2;
  if (choice==='le'      && !r.isEven && r.number!==0)   return amt*2;
  if (choice==='thap'   && r.range==='low')              return amt*2;
  if (choice==='cao'    && r.range==='high')             return amt*2;
  if (choice==='ta1'    && r.dozen===1)                  return amt*3;
  if (choice==='ta2'    && r.dozen===2)                  return amt*3;
  if (choice==='ta3'    && r.dozen===3)                  return amt*3;
  return 0;
}
